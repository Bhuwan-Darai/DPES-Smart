...
import NewsFeedForm, {
  InitialData,
} from "@/components/principal/Newsfeed/NewsFeedForm";
import { DELETE_NEWSFEED } from "@/lib/hooks/graphql/PrincipalQueries";
...
export default function FeedScreen() {
  ...
  const [editPost, setEditPost] = useState<NewsFeed | null>(null);
  ...
  // delete newsfeed
  const [deleteNewsFeed] = useMutation(DELETE_NEWSFEED, {
    onCompleted: (data) => {
      Alert.alert(data.deleteNewsfeed.message);
      console.log("data delete newsfeed", data);
      refetchNewsfeed();
    },
    onError: (error) => {
      Alert.alert(error.message);
      console.log("error delete newsfeed", error);
    },
  });
    ...
    return (
      <View key={post.newsFeedId} style={styles.postCard}>
        <View style={styles.postHeader}>
          ...
          <View>
            <TouchableOpacity
              onPress={() => setDropdownVisible(post.newsFeedId)}
            >
              <MoreHorizontal size={20} color="#666" />
            </TouchableOpacity>
            {dropdownVisible === post.newsFeedId && (
      ...
      {isDialogOpen && (
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={styles.newFeedModalContainer}>
              <View style={styles.newFeedModalHeader}>
                <View style={styles.modalTitleContainer}>
                  <Text style={styles.modalTitle}>Create Newsfeed</Text>
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalContainer}>
              <View style={styles.modalHeader}>
                ...
                <Text style={styles.modalSubtitle}>
                  Edit your newsfeed here
                </Text>
              </View>
              <NewsFeedForm
                initialData={editPost as unknown as InitialData}
