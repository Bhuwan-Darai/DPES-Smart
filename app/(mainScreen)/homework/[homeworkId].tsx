...
export default function HomeworkDetailScreen() {
  const { homeworkId, homework } = useLocalSearchParams();
  const homeworkData = homework ? JSON.parse(homework as string) : null;
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.statusBadge}>
            <View
              style={[
                ...
                {
                  backgroundColor: getStatusColor(homeworkData.dueDate),
                },
              ]}
            />
            <Text
              style={[
                styles.statusText,
                {
                  color: getStatusColor(homeworkData.dueDate),
                },
              ]}
            >
              {getStatusColor(homeworkData.dueDate) === "#FF3B30"
                ? "Overdue"
                : "Active"}
            </Text>
          </View>
        ...
        <View style={styles.detailRow}>
          ...
          <View style={styles.detailContent}>
            ...
            <Text style={styles.detailValue}>
              {formatDate(homeworkData.createdAt)}
            </Text>
          </View>
        </View>
        <View style={styles.detailRow}>
          ...
          <View style={styles.detailContent}>
            ...
            <Text style={[styles.detailValue, styles.dueDate]}>
              {formatDate(homeworkData.dueDate)}
            </Text>
          </View>
        </View>
      ...
      {/* Homework Files */}
      {homeworkData.homeworkFiles && homeworkData.homeworkFiles.length > 0 && (
        <View style={styles.card}>
          ...
          <View style={styles.filesContainer}>
            {homeworkData.homeworkFiles.map((file: any, index: number) => (
              <TouchableOpacity
                key={file.fileId}
                style={[
                  styles.fileCard,
                  {
                    marginBottom:
                      index === homeworkData.homeworkFiles.length - 1 ? 0 : 12,
                  },
                ]}
      ...
      {isSubmitted && homeworkData.submissions && (
        <View style={styles.card}>
          ...
          {homeworkData.submissions.map((submission: any) => (
