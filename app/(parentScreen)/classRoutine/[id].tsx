...
import { GET_CLASS_ROUTINE_BY_PARENT } from "@/lib/hooks/graphql/ParentQueries";
...
export default function TimetableScreen() {
  ...
  console.log("data", JSON.stringify(data?.getClassRoutineByParent, null, 2));
  ...
  const getAllLessons = (): Lesson[] => {
    if (!data?.getClassRoutineByParent) return [];

    const lessons: Lesson[] = [];

    // Add periods (classes)
    data.getClassRoutineByParent.periods.forEach((period: Period) => {
    ...
    // Add breaks
    if (
      data.getClassRoutineByParent.breakTime &&
      Array.isArray(data.getClassRoutineByParent.breakTime)
    ) {
      data.getClassRoutineByParent.breakTime.forEach((breakTime: BreakTime) => {
        lessons.push({
        ...
        <View style={styles.routineHeader}>
          <View style={styles.headerContent}>
            <Text style={styles.routineTitle}>
              {data?.getClassRoutineByParent?.routineName || "Today's Schedule"}
            </Text>
