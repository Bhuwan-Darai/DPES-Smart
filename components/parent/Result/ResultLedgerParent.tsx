...
const ResultLedgerParent = () => {
  ...
  const {
    ...
    error: ledgerError,
  } = useQuery<ResultLedgerResponse>(RESULT_LEDGER_PARENT, {
    variables: {
      examId: selectedExam,
      page,
      limit,
    ...
    skip: !selectedExam,
    onCompleted: (data) => {
      if (data?.resultLedgerParent?.ledger) {
        const newLedgerData = data.resultLedgerParent.ledger;
        ...
        const totalPages = data.resultLedgerParent.totalPages;
        setHasMore(page < totalPages);
        setTotalStudents(data.resultLedgerParent.total);
      }
  ...
  const loadMoreData = useCallback(() => {
    if (!isLoadingMore && hasMore && selectedExam) {
      setIsLoadingMore(true);
      const nextPage = page + 1;
      setPage(nextPage);
    }
  }, [isLoadingMore, hasMore, selectedExam, page]);
...
export default ResultLedgerParent;
