...
interface HomeworkItem {
  ...
  dueTime: string | Date;
  points: string;
  homeworkFiles: HomeworkFileInput[];
}
...
const HomeworkManager = () => {
  ...
  console.log("subjectDatas", JSON.stringify(subjectDatas, null, 2));
  console.log("classDatas", JSON.stringify(classDatas, null, 2));
  console.log("sectionDatas", JSON.stringify(sectionDatas, null, 2));
  ...
  const {
    ...
    refetch,
    fetchMore,
  } = useQuery(GET_HOMEWORKS_BY_TEACHER, {
    variables: {
      filters: {
  ...
  const [
    createHomework,
    { loading: createHomeworkLoading, error: createHomeworkError },
  ] = useMutation(CREATE_HOMEWORK_MUTATION);
  const [
    updateHomework,
    { loading: updateHomeworkLoading, error: updateHomeworkError },
  ] = useMutation(UPDATE_HOMEWORK_MUTATION);
  const [
    deleteHomework,
    { loading: deleteHomeworkLoading, error: deleteHomeworkError },
  ] = useMutation(DELETE_HOMEWORK_MUTATION);

  console.log(
    "createHomeworkError",
    JSON.stringify(createHomeworkError, null, 2)
  );
  console.log(
    "updateHomeworkError",
  ...
  // Set homework list
  useEffect(() => {
    if (homeworksData?.getHomeworksByTeacher?.homeworks) {
      setHomeworkList(homeworksData.getHomeworksByTeacher.homeworks);
    }
  }, [homeworksData]);
  ...
  const handleDateChange = useCallback(
    ...
    [formData.dueDate, handleInputChange]
  );
    ...
    try {
      ...
      if (result.assets && result.assets.length > 0) {
        ...
        const uploadPromises = result.assets.map(async (file) => {
          ...
          const folderPath = `homework/dailyHomework`;
      ...
      if (result && !("cancel" in result)) {
        handleInputChange("homeworkFiles", result);
      }
    } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    await createHomework({
      ...
      onCompleted: (data) => {
        if (data.createHomework.success === true) {
          Alert.alert("Success", "Homework added successfully!");
          handleModalClose();
          refetch();
        } else {
          Alert.alert("Error", data.createHomework.message);
        }
      },
      onError: (error) => {
  ...
  const handleEdit = (homework: any) => {
    ...
    setFormData({
      ...
      dueTime: homework.dueDate,
      points: homework.points.toString(),
      homeworkFiles: homework.homeworkFiles || [],
    });
    setFilePath(
      homework.homeworkFiles?.map((file: HomeworkFile) => ({
        fileId: file.fileId,
  ...
  const handleUpdate = async () => {
    ...
    const dataToSend = {
      ...
      description: formData.description,
      dueDate: formData.dueDate.toISOString().split("T")[0],
      dueTime: getLocalTimeHHMMSS(new Date(formData.dueTime)),
      points: Number(formData.points),
      homeworkFiles: filePath.map((file) => {
        if (typeof file === "string") {
          // This is a new file
    ...
    await updateHomework({
      variables: { input: dataToSend },
      onCompleted: (data) => {
        if (data.updateHomework.success === true) {
          Alert.alert("Success", "Homework updated successfully!");
          setShowAddForm(false);
          setIsEditing(false);
          refetch();
        } else {
          Alert.alert("Error", data.updateHomework.message);
        }
      },
      onError: (error) => {
        Alert.alert("Error", error.message);
  ...
  const confirmDelete = async () => {
    await deleteHomework({
      ...
      onCompleted: (data) => {
        if (data.removeHomework.success === true) {
          Alert.alert("Success", "Homework deleted successfully!");
          setShowDeleteConfirm(false);
          refetch();
        } else {
          Alert.alert("Error", data.removeHomework.message);
        }
      },
      onError: (error) => {
        Alert.alert("Error", error.message);
    ...
    return (
      <View style={styles.homeworkCard}>
        ...
        <View className="flex-row justify-between items-center mb-4">
          <View>
            ...
            <Text className="text-sm font-semibold text-gray-800">
              {formatDate(new Date(item.dueDate))} at{" "}
              {formatTime(new Date(item.dueDate))}
            </Text>
          </View>
          {item.homeworkFiles && item.homeworkFiles.length > 0 && (
            <View style={styles.attachmentIndicator}>
              <Text className="text-green-600 text-xs font-medium">
                📎 {item.homeworkFiles.length} Attachment(s)
              </Text>
            </View>
          )}
        </View>
        ...
        <View className="flex-row gap-3 mb-4">
          ...
          >
            <Text className="text-white s font-semibold">Edit</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.actionButton, styles.deleteButton]}
            onPress={() => handleDelete(item.homeworkId)}
          >
        ...
        {item.homeworkFiles && item.homeworkFiles.length > 0 && (
          <View className="mt-2">
            ...
            {item.homeworkFiles.map((file: any, index: number) => (
              <TouchableOpacity
                key={index}
  ...
  const loadMoreHomeworks = () => {
    if (loading || !homeworksData) return;

    const total = homeworksData.getHomeworksByTeacher.total;
        ...
        updateQuery: (prevResult, { fetchMoreResult }) => {
          ...
          return {
            ...
            getHomeworksByTeacher: {
              ...prevResult.getHomeworksByTeacher,
              homeworks: [
                ...prevResult.getHomeworksByTeacher.homeworks,
                ...fetchMoreResult.getHomeworksByTeacher.homeworks,
              ],
            },
          };
        },
          ...
          <ScrollView style={styles.formContainer}>
            ...
            <View className="flex-row gap-3 mb-4">
              ...
              <View className="flex-1">
                ...
                >
                  <Text className="text-gray-800 font-medium">
                    {formatTime(new Date(formData.dueTime))}
                  </Text>
                </TouchableOpacity>
              </View>
            </View>
            ...
            <TouchableOpacity
              ...
              disabled={
                isEditing ? updateHomeworkLoading : createHomeworkLoading
              }
            >
